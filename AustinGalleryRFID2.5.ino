...
const int threshold = 100; //photosensor threshold

//hall effect
const byte hall = A3;

//Artifact 0 Broken Atlas pin 2 (not using this ATM)
...
//Artifact 1 ELEPHANT pin 3 (using this)
...
void loop() {
  ...
  for (uint8_t i = 0; i < numReaders; i++) {
    ...
    if (readRFID[i] != currentIDs[i]) {
      ...
      if (currentIDs[1] == tag10 || currentIDs[1] == tag11 || currentIDs[1] == tag12 || currentIDs[1] == tag13) {
        sequenceID[1] = 2;
        digitalWrite(buzzer, HIGH);
        delay(50);
        digitalWrite(buzzer, LOW);
      ...
      if (currentIDs[2] == tag40 || currentIDs[2] == tag41 || currentIDs[2] == tag42 || currentIDs[2] == tag43) {
        sequenceID[2] = 3;
        digitalWrite(buzzer, HIGH);
        delay(50);
        digitalWrite(buzzer, LOW);
